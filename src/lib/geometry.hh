...
public:
	...
	Vector(const Vector &p0, const Vector &p1) : x(p1.x - p0.x), y(p1.y - p0.y), z(p1.z - p0.z)
	{
	}

	explicit Vector(const T *p) : x(p[0]), y(p[1]), z(p[2]) {}
...
public:
	...
	Vector2D<T>(const Vector2D<T> &p0, const Vector2D<T> &p1) : x(p1.x - p0.x), y(p1.y - p0.y) {}
